...
public partial class contenido_SysAdmin_ListaRoles : System.Web.UI.Page
{
    ClassRoles pers = new ClassRoles();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            try
            {
                ...
                lsPer = modfunc.fnValidaUsrApp("MANT_ROLES", gUsr, asCodSistema);
    ...
    protected void dgData_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        Response.Redirect("~/contenido/SysAdmin/GestionRoles.aspx?key=" + dgData.DataKeys[dgData.SelectedIndex].Values[0].ToString());
    }
    ...
    private void mfBuscar()
    {
        ...
        ds = pers.mfBuscar(this.txtBNom.Text, Session["codHosp"].ToString(), lsIdEstado, Session["user"].ToString(), System.Environment.MachineName);
        ...
        if (ds != null && ds.Tables.Count > 0)
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                this.dgData.Caption = "Listado Roles";
            }
            else
            {
    ...
    {
        Response.Redirect("~/contenido/SysAdmin/GestionRoles.aspx?key=0");
    }
