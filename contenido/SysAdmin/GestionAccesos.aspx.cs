...
public partial class contenido_SysAdmin_GestionAccesos : System.Web.UI.Page
{
    Mensaje mens = new Mensaje();
    ClassAccesos acs = new ClassAccesos();
    ...
    {
        Response.Redirect("~/contenido/SysAdmin/ListaAccesos.aspx?id=0");
    }
    private void mfAgregar()
    {
        ...
        if (largo <= 900)
        {
            ...
            else
            {
                ...
                mfCargaUser();
                mfCargaUserDisp();
                mens.mensaje(Page, "Invitado Ingresado Exitosamente.. ");
            }
        }
        else
        {
    ...
    {
        DataSet aoDs;
        aoDs = acs.jaxListaRoles(Session["codHosp"].ToString(), Session["lsIdentificador"].ToString(), TNombreRol.Text);
    ...
    {
        DataSet aoDs;
        aoDs = acs.jaxListaRolesDisp(Session["codHosp"].ToString(), Session["lsIdentificador"].ToString(), TNombreRol.Text);
        gdArt.DataSource = aoDs;
        gdArt.DataBind();
    }
    private void mfAgregarRol(string asIdentificador)
    {
        string lsRet = "";
        lsRet = acs.jaxAgregarRol(Session["lsIdentificador"].ToString(), asIdentificador, this.rbTipo.SelectedValue, Session["codHosp"].ToString(), Session["user"].ToString(), System.Environment.MachineName);
        if (lsRet != "")
        {
            mens.mensaje(Page, "Error: NO se pudo insertar Rol");
        }
        else
        {
            mfCargaGrp();
    ...
    private void mfElim(string asIdentificador)
    {
        ...
        if (lsExiste == "0")
        {
            ...
            if (lsRet != "")
            {
                mens.mensaje(Page, "Error: NO se pudo Eliminar Rol");
            }
            else
            {
                mfCargaGrp();
        ...
        {
            mens.mensaje(Page, "Rol se Encuentra asignado ");
        }
    }
    protected void gdArt_SelectedIndexChanged(object sender, EventArgs e)
    {
        mfAgregarRol(gdArt.DataKeys[gdArt.SelectedIndex].Values[0].ToString());
    }
    ...
    {
        DataSet aoDs;
        aoDs = acs.jaxListaUsuarios(Session["codHosp"].ToString(), Session["lsIdentificador"].ToString(), this.TNombreUsr.Text, this.TRut.Text);
